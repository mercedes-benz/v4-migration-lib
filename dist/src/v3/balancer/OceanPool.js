...
export var PoolCreateProgressStep
;(function (PoolCreateProgressStep) {
  ...
  PoolCreateProgressStep[(PoolCreateProgressStep['SetupPool'] = 3)] =
    'SetupPool'
})(PoolCreateProgressStep || (PoolCreateProgressStep = {}))
/**
...
export class OceanPool extends Pool {
  ...
  /**
       ...
       * @return {String}
       */
  create(account, dtAddress, dtAmount, dtWeight, oceanAmount, fee) {
   ...
   * @return {string}
   */
  async getDTAddress(poolAddress) {
    this.dtAddress = null
   ...
   * @return {String}
   */
  async getOceanReserve(poolAddress) {
    if (this.oceanAddress == null) {
      this.logger.error('ERROR: oceanAddress is not defined')
      return null
    }
   ...
   * @return {String}
   */
  async getDTReserve(poolAddress) {
   ...
   * @param tokenAddress
   */
  async getMaxBuyQuantity(poolAddress, tokenAddress) {
   ...
   * @param tokenAddress
   */
  async getOceanMaxBuyQuantity(poolAddress) {
   ...
   * @param tokenAddress
   */
  async getDTMaxBuyQuantity(poolAddress) {
   ...
   * @param tokenInAmount
   */
  async calcPoolOutGivenSingleIn(poolAddress, tokenInAddress, tokenInAmount) {
   ...
   * @param poolShares
   */
  async calcSingleInGivenPoolOut(poolAddress, tokenInAddress, poolShares) {
   ...
   * @param poolShares
   */
  async calcSingleOutGivenPoolIn(poolAddress, tokenOutAddress, poolShares) {
   ...
   * @param tokenOutAmount
   */
  async calcPoolInGivenSingleOut(poolAddress, tokenOutAddress, tokenOutAmount) {
   ...
   * @param dtAmount
   */
  async getPoolSharesRequiredToRemoveDT(poolAddress, dtAmount) {
    const dtAddress = await this.getDTAddress(poolAddress)
   ...
   * @param poolShares
   */
  async getDTRemovedforPoolShares(poolAddress, poolShares) {
    const dtAddress = await this.getDTAddress(poolAddress)
   ...
   * @param dtAmount
   */
  async getPoolSharesRequiredToRemoveOcean(poolAddress, oceanAmount) {
   ...
   * @param poolShares
   */
  async getOceanRemovedforPoolShares(poolAddress, poolShares) {
   ...
   */
  async getTokensRemovedforPoolShares(poolAddress, poolShares) {
    try {
   ...
   * @param poolAddress
   */
  async getDTMaxAddLiquidity(poolAddress) {
    const dtAddress = await this.getDTAddress(poolAddress)
   ...
   * @param poolAddress
   */
  async getOceanMaxAddLiquidity(poolAddress) {
   ...
   * @param tokenAddress
   */
  async getMaxAddLiquidity(poolAddress, tokenAddress) {
    const balance = await super.getReserve(poolAddress, tokenAddress)
   ...
   * @param tokenAddress
   */
  async getMaxRemoveLiquidity(poolAddress, tokenAddress) {
    const balance = await super.getReserve(poolAddress, tokenAddress)
    if (parseFloat(balance) > 0) {
   ...
   * @param tokenAddress
   */
  async getDTMaxRemoveLiquidity(poolAddress) {
    const dtAddress = await this.getDTAddress(poolAddress)
   ...
   * @param tokenAddress
   */
  async getOceanMaxRemoveLiquidity(poolAddress) {
   ...
   */
  async buyDT(account, poolAddress, dtAmountWanted, maxOceanAmount, maxPrice) {
    if (this.oceanAddress == null) {
   ...
   * @return {TransactionReceipt}
   */
  async sellDT(account, poolAddress, dtAmount, oceanAmountWanted, maxPrice) {
    if (this.oceanAddress == null) {
      this.logger.error('ERROR: oceanAddress is not defined')
      return null
    }
   ...
   * @return {TransactionReceipt}
   */
  async addDTLiquidity(account, poolAddress, amount) {
    const dtAddress = await this.getDTAddress(poolAddress)
   ...
   * @return {TransactionReceipt}
   */
  async removeDTLiquidity(account, poolAddress, amount, maximumPoolShares) {
    const dtAddress = await this.getDTAddress(poolAddress)
   ...
   * @return {TransactionReceipt}
   */
  async addOceanLiquidity(account, poolAddress, amount) {
    if (this.oceanAddress == null) {
      this.logger.error('ERROR: oceanAddress is not defined')
      return null
    }
   ...
   * @return {TransactionReceipt}
   */
  async removeOceanLiquidityWithMinimum(
    account,
    poolAddress,
    poolShares,
    minOcean
   ...
   * @return {TransactionReceipt}
   */
  async removeOceanLiquidity(account, poolAddress, amount, maximumPoolShares) {
    if (this.oceanAddress == null) {
      this.logger.error('ERROR: oceanAddress is not defined')
      return null
    }
   ...
   * @return {String[]}
   */
  async searchPoolforDT(dtAddress) {
    const result = []
  ...
  async getOceanNeeded(poolAddress, dtRequired) {
    const dtAddress = await this.getDTAddress(poolAddress)
    if (
   ...
   */
  async getOceanReceived(poolAddress, dtAmount) {
    const dtAddress = await this.getDTAddress(poolAddress)
   ...
   * @return {String[]} - amount of ocean tokens received
   */
  async getDTReceived(poolAddress, oceanAmount) {
    const dtAddress = await this.getDTAddress(poolAddress)
    return this.calcOutGivenIn(
      poolAddress,
      this.oceanAddress,
  ...
  async getDTNeeded(poolAddress, OceanRequired) {
    const dtAddress = await this.getDTAddress(poolAddress)
    if (
   ...
   * @return {PoolDetails[]}
   */
  async getPoolsbyCreator(account) {
    const result = []
    const factory = setContractDefaults(
      new this.web3.eth.Contract(this.factoryABI, this.factoryAddress),
      this.config
   ...
   */
  async getPoolSharesByAddress(account) {
    const result = []
    const factory = setContractDefaults(
      new this.web3.eth.Contract(this.factoryABI, this.factoryAddress),
      this.config
  ...
  async getPoolDetails(poolAddress) {
    ...
    const details = { poolAddress, tokens }
    return details
  }
  /**
   ...
   */
  async getPoolLogs(poolAddress, startBlock = 0, account) {
    const results = []
    const dtAddress = await this.getDTAddress(poolAddress)
  ...
  async getEventData(poolAddress, dtAddress, data) {
  ...
  async computeBuySlippage(poolAddress, oceanAmount) {
    const dtAddress = await this.getDTAddress(poolAddress)
  ...
  async computeSellSlippage(poolAddress, dtAmount) {
    const dtAddress = await this.getDTAddress(poolAddress)
    const dtWeight = await super.getDenormalizedWeight(poolAddress, dtAddress)
    const oceanWeight = await super.getDenormalizedWeight(
      poolAddress,
