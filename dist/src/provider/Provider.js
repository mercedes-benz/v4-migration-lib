...
export class Provider {
   ...
   */
  async getEndpoints(providerUri) {
    try {
  ...
  getEndpointURL(servicesEndpoints, serviceName) {
   ...
   * @return {Promise<ServiceEndpoint[]>}
   */
  async getServiceEndpoints(providerEndpoint, endpoints) {
  ...
  async createSignature(web3, accountId, agreementId) {
    const signature = await signText(web3, noZeroX(agreementId), accountId)
    return signature
  }
  async createHashSignature(web3, accountId, message) {
   ...
   * @return {Promise<string>} urlDetails
   */
  async encrypt(data, providerUri, signal) {
   ...
   */
  async checkDidFiles(did, serviceId, providerUri, signal) {
    const providerEndpoints = await this.getEndpoints(providerUri)
    const serviceEndpoints = await this.getServiceEndpoints(
      providerUri,
      providerEndpoints
   ...
   * @return {Promise<FileMetadata[]>} urlDetails
   */
  async checkFileUrl(url, providerUri, signal) {
    const providerEndpoints = await this.getEndpoints(providerUri)
    const serviceEndpoints = await this.getServiceEndpoints(
      providerUri,
      providerEndpoints
   ...
   */
  async getComputeEnvironments(providerUri, signal) {
    const providerEndpoints = await this.getEndpoints(providerUri)
    const serviceEndpoints = await this.getServiceEndpoints(
      providerUri,
      providerEndpoints
   ...
   * @return {Promise<ComputeJob | ComputeJob[]>}
   */
  async computeStop(did, consumerAddress, jobId, providerUri, web3, signal) {
    const providerEndpoints = await this.getEndpoints(providerUri)
    const serviceEndpoints = await this.getServiceEndpoints(
      providerUri,
      providerEndpoints
   ...
   * @return {Promise<ComputeJob | ComputeJob[]>}
   */
  async computeStatus(providerUri, signal, jobId, did, consumerAddress) {
   ...
   * @return {Promise<ComputeJob | ComputeJob[]>}
   */
  async computeResult(jobId, index, accountId, providerUri, web3, signal) {
    const providerEndpoints = await this.getEndpoints(providerUri)
    const serviceEndpoints = await this.getServiceEndpoints(
      providerUri,
      providerEndpoints
   ...
   * @return {Promise<ComputeJob | ComputeJob[]>}
   */
  async computeDelete(did, consumerAddress, jobId, providerUri, web3, signal) {
    const providerEndpoints = await this.getEndpoints(providerUri)
    const serviceEndpoints = await this.getServiceEndpoints(
      providerUri,
      providerEndpoints
   ...
   */
  async isValidProvider(url, signal) {
    try {
